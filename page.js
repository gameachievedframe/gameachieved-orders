...
// ⚠️⚠️⚠️ CONFIGURATION EMAILJS - REMPLACE PAR TES VRAIES CLÉS ⚠️⚠️⚠️
...
// ⚠️ REMPLACE PAR TON LIEN PAYPAL
const PAYPAL_USERNAME = 'tornusin';  // Ex: johnsmith

// ⚠️ REMPLACE PAR TON LIEN IMGUR DU LOGO
...
export default function GameAchievedOrderSystem() {
  ...
  const loadOrders = async () => {
    try {
      ...
      console.log('Première utilisation, aucune commande');
    }
  };
  ...
  const sendEmail = async (templateId, params, toEmail) => {
    if (!window.emailjs) {
      console.log('EmailJS non chargé - email non envoyé');
      return;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      alert(`✅ Commande enregistrée !\n\nTotal: ${totalPrice.toFixed(2)}€\n\nVous allez recevoir un email de confirmation.\nVous allez être redirigé vers PayPal pour le paiement.`);
    ...
    try {
      ...
      if (result) {
        ...
        if (newStatus === 'paye') {
          ...
          alert(`✅ Commande marquée comme payée.\n📧 Email de confirmation envoyé à ${order.email}`);
          ...
          alert(`✅ Commande passée en production.\n📧 Email envoyé à ${order.email}`);
          ...
          alert(`✅ Commande marquée comme expédiée.\n📧 Email d'expédition envoyé à ${order.email}`);
        }
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">Cadres personnalisés</p>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-lg p-8">
              <div className="flex items-start gap-6 mb-8">
                <div className="w-48 h-64 bg-gradient-to-br from-gray-700 to-gray-900 rounded-lg shadow-md flex items-center justify-center border-4 border-gray-400 relative">
                  ...
                  <div className="absolute bottom-2 left-2 w-6 h-6 bg-gray-500 rounded flex items-center justify-center text-xs">❤️</div>
                  <div className="absolute bottom-2 right-2 w-6 h-6 bg-gray-500 rounded flex items-center justify-center text-xs">⚡</div>
                </div>
                <div className="flex-1">
                  <h2 className="text-3xl font-bold text-gray-900 mb-2">Nouvelle commande</h2>
                  <p className="text-gray-600 mb-4">Cadre GameAchieved personnalisé - {PRICE_PER_FRAME}€/pièce</p>
                  <p className="text-sm text-gray-500">Dimensions : 15 x 20 cm</p>
                  <p className="text-sm text-gray-500">Design métallique avec trophée et inscription "GAME ACHIEVED"</p>
                </div>
              </div>
              ...
              <div className="space-y-6">
                ...
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">
                    Nom(s) du/des jeu(x) à personnaliser
                  </label>
                ...
                <div className="bg-indigo-50 rounded-lg p-4 border-2 border-indigo-200">
                  <div className="flex justify-between items-center text-lg font-bold text-indigo-900">
                    ...
                    <span className="text-2xl">{(PRICE_PER_FRAME * formData.quantity).toFixed(2)}€</span>
                  </div>
                </div>
          ...
          <div>
            <div className="bg-white rounded-2xl shadow-lg p-8">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-3xl font-bold text-gray-900">Commandes reçues</h2>
                  ...
                  {orders.map((order) => (
                    <div key={order.id} className="border-2 border-gray-200 rounded-xl p-6 hover:border-indigo-300 transition">
                      ...
                      <div className="bg-gray-50 rounded-lg p-4 mb-4">
                        <p className="font-semibold text-gray-700 mb-2">Jeux à personnaliser :</p>
